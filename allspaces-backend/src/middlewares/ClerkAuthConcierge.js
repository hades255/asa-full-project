...
import { verifyClerkSession, fetchClerkUserData } from "../utils/clerkApi.js";

dotenv.config();
const prisma = new PrismaClient();

export const authenticateConciergeUser = async (req, res, next) => {
  try {
    ...
    const responseUser = await fetchClerkUserData(session.user_id);

    console.log("Clerk User Response => ", responseUser);

    if (!responseUser) {
      return res.status(401).json({ error: "User doesn't exist on Clerk" });
    }

    if (responseUser?.unsafe_metadata?.type == "concierge") {
      next();
      return true;
    }
